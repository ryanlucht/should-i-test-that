...
must_haves:
  truths:
    - "User sees creator credits and vibe-coded attribution on all pages"
    - "User sees Hubbard book attribution in Basic mode calculator page only"
    - "User downloads PNG with descriptive filename (no timestamps)"
  ...
  artifacts:
    ...
    - path: "src/pages/CalculatorPage.tsx"
      provides: "Mode-aware Hubbard attribution, cleaned up unused refs"
...
<objective>
Update footer to show credits on all pages, improve PNG export filenames, and remove unused sectionRefs.

Purpose: Footer should display proper attribution (creator, AI tools, and Hubbard book in Basic mode). PNG exports should have descriptive filenames without timestamps. CalculatorPage has unused sectionRefs that should be removed for cleanliness.

Output: Footer visible on all pages with correct attribution, PNG exports with meaningful filenames, cleaner CalculatorPage code.
</objective>
...
3. Remove unused sectionRefs from CalculatorPage.tsx:
   - Remove the line `const sectionRefs = useRef<Record<string, HTMLElement | null>>({});` (around line 108)
   - Remove the ref callback in the section div: `ref={(el) => { sectionRefs.current[section.id] = el; }}` (around line 315-317)
   - The section div should just be: `<div onKeyDown={(e) => handleKeyDown(e, index)} className="min-h-32">`
  </action>
  <verify>Run `npm run dev`, verify: Welcome page has full footer, Basic mode calculator has full footer with Hubbard line, Advanced mode calculator has footer without Hubbard line. Run `npm run typecheck` to confirm no errors from removing sectionRefs.</verify>
  <done>Footer shows on all pages with correct attribution, Hubbard line only in Basic mode, unused sectionRefs removed</done>
</task>
...
4. Manual verification:
   ...
   - No unused sectionRefs in CalculatorPage code
</verification>
...
- Unused sectionRefs removed from CalculatorPage.tsx
</success_criteria>

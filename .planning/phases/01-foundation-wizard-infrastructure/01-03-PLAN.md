...
<task type="auto">
       ...
       export const useWizardStore = create<WizardState & WizardActions>()(
         persist(
           (set, get) => ({
             ...
             setMode: (mode) => {
               // When switching to basic, we keep shared inputs but don't need advanced
               // When switching to advanced, shared inputs carry over, advanced starts fresh if empty
               set({ mode });
             },
  ...
  <verify>
    ...
    ```
    - Click button, mode should change
  ...
  <done>
    Zustand store created with typed state for mode, inputs (shared + advanced), and navigation. Session persistence works for inputs and mode only.
  </done>
</task>
...
npm run build
npm run dev
# In browser: select Advanced, click Get Started, refresh, go back - Advanced still selected
```
</verification>

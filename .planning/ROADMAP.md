...
**Success Criteria** (what must be TRUE):
  1. Monte Carlo EVSI correctly uses threshold in payoff calculation (K * (L_true - T_L))
  2. Zero valid samples edge case returns safe "no information" result (EVSI=0)
  3. Worker lifecycle properly cleaned up on unmount and errors
  4. Advanced mode has default prior option matching Basic mode

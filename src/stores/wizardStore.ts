  ...
  persist(
    ...
    }),
    {
      ...
      merge: (persistedState: unknown, currentState: WizardStore): WizardStore => {
        const persisted = persistedState as Partial<Pick<WizardStore, 'inputs'>> | undefined;
        if (!persisted?.inputs) return currentState;

        // Only copy keys that exist in current initialInputs (strips obsolete fields)
        const cleanInputs = { ...currentState.inputs };
        ...
        return { ...currentState, inputs: cleanInputs };
      },
    }
  )
);

  ...
  describe('when mode is advanced', () => {
    ...
    describe('and all inputs are complete (Normal prior - fast path)', () => {
      ...
      it('includes directional probability fields (pStopsShip / pConvincesShip)', async () => {
        // Verifies that pStopsShip and pConvincesShip flow through hook → UI consumers.
        // Uses asymmetric prior so default=ship and pStopsShip is the active direction.
        act(() => {
          setupSharedInputs();
          setupAdvancedInputs();
          const { setSharedInput } = useWizardStore.getState();

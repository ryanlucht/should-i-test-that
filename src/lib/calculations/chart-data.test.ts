 ...
 *
 * Note: Legacy wrappers (generateDensityCurveData, getDensityAtLift) were
 * deleted in Cleanup-05. Tests now use the unified generateDistributionData
 * and getDensityAtLiftForPrior functions directly.
 */
...
describe('generateDistributionData', () => {
  describe('Normal distribution', () => {
    ...
    it('returns requested number of points', () => {
      ...
      const data = generateDistributionData(prior, 50);
    ...
    it('covers correct range: mu +/- 4 sigma', () => {
      ...
      const prior: PriorDistribution = { type: 'normal', mu_L, sigma_L };
      const data = generateDistributionData(prior);
...
describe('getDensityAtLiftForPrior', () => {
  it('returns correct density for Normal prior at the mean', () => {
    const prior: PriorDistribution = {
      type: 'normal',
      mu_L: 0.02,
      sigma_L: 0.05,
    ...
    const expectedDensity = (1 / SQRT_2_PI) / 0.05;

    expect(density).toBeCloseTo(expectedDensity, 10);
  });

  it('returns lower density at points away from mean for Normal prior', () => {
    const mu_L = 0;
    const sigma_L = 0.05;
    const prior: PriorDistribution = { type: 'normal', mu_L, sigma_L };

    const densityAtMean = getDensityAtLiftForPrior(mu_L, prior);
    const densityAtOneSigma = getDensityAtLiftForPrior(mu_L + sigma_L, prior);
    const densityAtTwoSigma = getDensityAtLiftForPrior(
      mu_L + 2 * sigma_L,
      prior
    );
  ...
  it('is symmetric around the mean for Normal prior', () => {
    const mu_L = 0.03;
    const sigma_L = 0.04;
    const offset = 0.02;
    const prior: PriorDistribution = { type: 'normal', mu_L, sigma_L };

    const densityAbove = getDensityAtLiftForPrior(mu_L + offset, prior);
    const densityBelow = getDensityAtLiftForPrior(mu_L - offset, prior);

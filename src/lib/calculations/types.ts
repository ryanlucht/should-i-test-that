...
export interface EVSIResults {
  ...
  /**
   * Probability test stops you from shipping (default=ship, posterior=dont-ship).
   * Directional split of probabilityTestChangesDecision.
   * Only non-zero when defaultDecision === 'ship'.
   */
  pStopsShip?: number;

  /**
   * Probability test convinces you to ship (default=dont-ship, posterior=ship).
   * Directional split of probabilityTestChangesDecision.
   * Only non-zero when defaultDecision === 'dont-ship'.
   */
